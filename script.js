        ...
        const top = setInterval(() => {
            ...
            if (Math.abs(camera.position.length() - zoomDistance) > zoomIncrement) { // set zoom distance
                ...
                Math.abs(new THREE.Vector3(0, 0, 0).copy(camera.position).projectOnPlane(j).angleTo(new THREE.Vector3(0, 0, 0).copy(i).negate())) > Math.PI/180) { // set the first angle
                    if (camera.position.x > 0 && camera.position.z > 0 && Math.abs(camera.position.x/camera.position.z) >= 1) {
...
function init() {
    ...
    const points = [new THREE.Vector3(0, 0.5, 0), new THREE.Vector3(0, 0, 0)];
    let fieldAxisGeometry = new THREE.BufferGeometry().setFromPoints(points);
    let fieldAxisMaterial = new THREE.LineBasicMaterial({color: '#ffffff'});
    fieldAxis = new THREE.LineSegments(fieldAxisGeometry, fieldAxisMaterial);
    scene.add(fieldAxis);
    const pointsVel = [new THREE.Vector3(0.5, 0, 0), new THREE.Vector3(0, 0, 0)];
    let velAxisGeometry = new THREE.BufferGeometry().setFromPoints(pointsVel);
    let velAxisMaterial = new THREE.LineBasicMaterial({color: '#03fc35'});
    velAxis = new THREE.LineSegments(velAxisGeometry, velAxisMaterial);
    scene.add(velAxis);
    let thetaGeometry = new THREE.CircleGeometry(0.4, 16, 0, Math.PI/2);
    let thetaMaterial = new THREE.MeshBasicMaterial({color: '#ffffff', transparent: true, opacity: 0.4});
    thetaMaterial.side = THREE.DoubleSide;
    theta = new THREE.Mesh(thetaGeometry, thetaMaterial);
    theta.renderOrder = -2;
    scene.add(theta);
    fontLoader.load('static/fonts/helvetiker_regular.typeface.json',
        (font) => {
            let angleGeometry = new TextGeometry('θ', {
                font: font,
                size: 0.16,
                depth: 0.00,
            });
            let angleMaterial = new THREE.MeshBasicMaterial({color: '#ffffff'});
            angleGeometry.computeBoundingBox();
            angleGeometry.translate(
                - angleGeometry.boundingBox.max.x * 0.5,
                - angleGeometry.boundingBox.max.y * 0.5,
                - angleGeometry.boundingBox.max.z * 0.5
            )
            angle = new THREE.Mesh(angleGeometry, angleMaterial);
            scene.add(angle);
        }
    );
    ...
    miscUI.add(visual, 'showTheta').name('มุมระหว่าง v และ B').listen();
    ...
    const teachingUI = gui.addFolder('Teaching Tools').close();
    teachingUI.add(teachingTools, 'handIsFlipped').name('พลิกมือให้ผิดด้าน');
...
function onEachStep() {
    ...
    // teaching tools
    if (teachingTools.handIsFlipped) {
        hand.rotateZ(Math.PI);
    }
    ...
    // angle between v and B
    if (visual.showTheta && charge.calcForce().length() != 0) {
        fieldAxis.visible = true;
        if (charge.velVect.visible) {
            velAxis.visible = false;
        } else {
            velAxis.visible = true;
        }
        theta.visible = true;
        angle.visible = true;
        fieldAxis.position.copy(charge.mesh.position);
        if (field.magnitude <= 0) {
            fieldAxis.translateY(-0.5);
        }
        velAxis.position.copy(charge.mesh.position);
        velAxis.lookAt(charge.velVectLabel.position);
        velAxis.rotateY(-Math.PI/2);
        theta.position.copy(charge.mesh.position);
        theta.lookAt(charge.velVectLabel.position);
        theta.rotateY(-Math.PI/2);
        updateTheta();
        if (angle) {
            angle.position.copy(charge.mesh.position);
            angle.lookAt(charge.velVectLabel.position);
            angle.rotateY(-Math.PI/2);
            if (field.magnitude > 0) {
                angle.position.add(new THREE.Vector3(0, 0, 0).copy(charge.velocity).normalize().lerp(j, 0.5).normalize().multiplyScalar(0.24));
            } else {
                angle.position.add(new THREE.Vector3(0, 0, 0).copy(charge.velocity).normalize().lerp(jNeg, 0.5).normalize().multiplyScalar(0.24));
            }
        }
    } else {
        fieldAxis.visible = false;
        velAxis.visible = false;
        theta.visible = false;
        if (angle) {
            angle.visible = false;
...
function updateTheta() {
    ...
    if (field.magnitude > 0) {
        if (charge.velocity.angleTo(j) == Math.PI/2) {
            theta.geometry = new THREE.PlaneGeometry(0.34, 0.34);
            theta.geometry.translate(0.17, 0.17, 0)
            return;
        }
        theta.geometry = new THREE.CircleGeometry(0.4, 16, 0, charge.velocity.angleTo(j));
    } else {
        if (charge.velocity.angleTo(j) == Math.PI/2) {
            theta.geometry = new THREE.PlaneGeometry(0.34, 0.34);
            theta.geometry.translate(0.17, -0.17, 0)
        ...
        theta.geometry = new THREE.CircleGeometry(0.4, 16, 0, -charge.velocity.angleTo(jNeg));
    }
}